...
export class MainScene extends AbstractGameScene {
    ...
    constructor(app: PIXI.Application) {
        ...
        for (let i = 0; i < NUM_CHESTS; i++) {
            ...
            this.chestsModel.push({
                position: {
                    x: (i + 1) % 2 ? this.WIDTH / 2 - Math.round(CONSTANTS.CHEST_WIDTH / 2) : this.WIDTH / 2,
                    y: (i + 1) % 2 ? Math.round(CHEST_HEIGHT * (i + 3) / 2) : Math.round(CHEST_HEIGHT * (i + 2) / 2)
                },
                size: {
                    width: Math.round(CONSTANTS.CHEST_WIDTH / 2) - 10,
                    height: CHEST_HEIGHT - 10
                },
                index: i,
                activated: false
            });
        ...
        this.playText.x = (this.WIDTH - CONSTANTS.CHEST_WIDTH/2) / 2 + Math.round((this.playBtn.width - this.playText.width) / 2);
        this.playText.y = this.HEIGHT - CHEST_HEIGHT - Math.round((this.playBtn.height + (this.playText.height)) / 2);
    ...
    addChestListeners(i: number) {
        this.chests[ i ].addListener("pointerdown", () => {
            ...
            this.currentWinChest.scale.set(0.2);
    ...
    drawPlayButton(color: number) {
        ...
        this.playBtn.drawRect(
            (this.WIDTH - CONSTANTS.CHEST_WIDTH/2) / 2,
            this.HEIGHT - 2 * CHEST_HEIGHT,
            CONSTANTS.CHEST_WIDTH / 2,
            CHEST_HEIGHT
        );
    ...
    sceneUpdate(delta: number) {
        ...
        this.currentWinChest.scale.x += 0.03 * delta;
        this.currentWinChest.scale.y += 0.03 * delta;
        
        if (this.currentWinChest.scale.x > 1) {
            this.currentWinChest = new PIXI.Text('');
